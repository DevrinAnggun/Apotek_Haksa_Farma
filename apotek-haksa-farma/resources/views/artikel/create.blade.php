...
<div class="mb-8 flex items-center gap-4">
    ...
    <div>
        <h1 class="text-2xl font-extrabold text-gray-800 tracking-tight">Tulis Artikel Baru</h1>
        <p class="text-gray-500 text-sm mt-1">Berikan informasi bermanfaat bagi pelanggan Anda.</p>
    </div>
</div>

<div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
    <form action="{{ route('artikel.store') }}" method="POST" enctype="multipart/form-data" class="p-8">
        @csrf
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
            ...
            <div>
                ...
                <div class="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-gray-300 border-dashed rounded-2xl hover:border-green-400 transition cursor-pointer relative" id="dropzone">
                    ...
                    <img id="preview" class="hidden absolute inset-0 w-full h-full object-cover rounded-2xl p-1 bg-white">
                </div>
            </div>
        </div>
        ...
        <div class="mb-8">
            ...
            <textarea name="ringkasan" rows="2" class="w-full px-4 py-3 rounded-xl border border-gray-200 focus:outline-none focus:ring-2 focus:ring-green-500 transition" placeholder="Tuliskan 1-2 kalimat pengantar artikel ini...">{{ old('ringkasan') }}</textarea>
        </div>

        {{-- Konten --}}
        <div class="mb-8">
            <label class="block text-sm font-bold text-gray-700 mb-2 uppercase tracking-wider">Isi Lengkap Artikel</label>
            <textarea name="konten" rows="12" required class="w-full px-4 py-3 rounded-xl border border-gray-200 focus:outline-none focus:ring-2 focus:ring-green-500 transition" placeholder="Tuliskan isi artikel selengkap mungkin...">{{ old('konten') }}</textarea>
        </div>
        ...
        <div class="flex items-center justify-end gap-3 pt-6 border-t border-gray-100 mt-8">
            ...
            <button type="submit" class="bg-green-600 hover:bg-green-700 text-white font-bold py-2.5 px-8 rounded-xl shadow-lg shadow-green-200 transition-all transform hover:-translate-y-0.5">
                Simpan & Publis Artikel
            </button>
        </div>
    </form>
</div>
